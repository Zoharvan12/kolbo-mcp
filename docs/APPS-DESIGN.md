...
|---|---|---|
| `ui://kolbo/generation.html` | generation and edit tools | Kolbo glass card: logo header, model chip (+icon), settings chips (duration/resolution/audio/count), one thumbnail for every reference image used, shimmer skeletons + spinner while generating → image grid / video player / responsive audio-track rows / 3D file cards. Every generated audio URL gets its own native player, title/track number, optional per-track metadata, and visible Download button. Result gallery actions include **Animate · Edit · Recreate · Download · Open in Kolbo** where applicable. Creative Director uses its dedicated batch-status tool. A `prompts[]` batch stays ONE grouped tile grid from generating through completed — every tile keeps the prompt that produced it as its caption, and downloads are per-tile; it never collapses into the single-item scenes carousel. All tracking uses one server-side long wait at a time and remains active for 35 minutes; losing tracking never offers an unsafe paid retry. Cards are offscreen-gated: a card polls for status (and therefore fetches media) only once it scrolls into view, so a long session does not fire one status call and one full-res download per generation on open. |
