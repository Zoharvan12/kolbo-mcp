...
**Timeout convention (v1.50+):** every tool that polls internally (all `generate_*`/`edit_*`/`transcribe_audio`/`chat_send_message`) routes its poll through `pollOrTimedOut()` (`src/tools/_shared.js`). If the "Timeout" window below elapses before the job is terminal, the tool returns a non-throwing `{ state: 'processing', generation_id, _timed_out: true, _hint }` result instead of throwing `PollingTimeoutError`. The generation continues running server-side, and the caller must poll `get_generation_status(generation_id, wait: true)` to resolve it; never re-run the same tool from scratch on a timeout.

**NOTE:** All generation tool timeouts are now 150s (2.5 minutes) to ensure they return before MCP host timeouts (~180s). Jobs that need longer (video, music, transcription) will return with `_timed_out: true` and the LLM should call `get_generation_status` with `wait: true` to continue polling. This prevents host timeouts from cancelling server-side generations.
...
| `generate_elements` | `POST /v1/generate/elements` | 150s | `reference_images`, `reference_videos`, `reference_audio_urls`, legacy `audio_url`, `files`, `visual_dna_ids`, `motion`, `preset_id`, `resolution`, `keyframes` (v1.56+), `multi_shots` / `multi_shot_count` / `session_name` (v1.65+) |
| `generate_first_last_frame` | `POST /v1/generate/first-last-frame` | 150s | URLs OR local paths for `first_frame`/`last_frame`, `visual_dna_ids`, `resolution` |
| `generate_lipsync` | `POST /v1/generate/lipsync` | 150s | `source` (URL or local), `audio` (URL or local), `bounding_box_target`; Sync-3 only: `sync_mode`, `model_mode`, `emotion`, `temperature`, `occlusion_detection_enabled`, `active_speaker_detection` |
| `generate_creative_director` | `POST /v1/generate/creative-director` | 150s | `visual_dna_ids`, `moodboard_id`, `moodboard_ids`, `reference_images`, `scene_count`, `workflow_type`, `resolution` |
| `generate_music` | `POST /v1/generate/music` | 150s | `lyrics`, `style`, `instrumental`, `vocal_gender`, `title`, `negative_tags`, `duration_seconds`; Suno fine-controls: `style_weight`, `weirdness`, `audio_weight`, `persona_id`, `use_composition_plan`, `singing_dna_id`, `singing_voice_id` |
| `generate_speech` | `POST /v1/generate/speech` | 150s | `voice` (id OR display name), `language` (Google = Accent); expressive/style controls (provider-specific, engine ignores non-applicable): Google/Gemini `style_instructions_preset_id` (warm/dramatic/whisper/excited/calm/cheerful/serious/storyteller/sad/intimate/british/commercial/custom/none) + optional `style_instructions`/`style_instructions_label`, `selected_style` (DeepDub / MiniMax / Cartesia), `emotion` (MiniMax/Cartesia), `speaking_speed`; ElevenLabs `similarity_boost`/`style`/`use_speaker_boost`; DeepDub `variance`/`tempo`/`promptBoost`/`seed`/`accentControl`/`voiceTitle`; MiniMax `minimax_pitch`/`minimax_vol`/`minimax_intensity`/`minimax_timbre`. Status result also returns these fields for reuse. |
| `generate_sound` | `POST /v1/generate/sound` | 150s | `duration`, `prompt_influence`; provider-specific (engine ignores non-applicable): `cfg_strength` (Stable Audio); `sound_loop`/`sound_tempo`/`sound_key` (Kie); Seed-Audio `seed_voice`/`seed_speed`/`seed_volume`/`seed_pitch`/`seed_reference_audio_urls`/`seed_reference_image_url` |
| `generate_3d` | `POST /v1/generate/3d` | 150s | `reference_images`, `mode` (text/single/multi), `topology`, `enable_pbr` |
| `transcribe_audio` | `POST /v1/transcribe` | 150s | `source` (URL or local audio/video); `language`, `diarize` (speaker labels), `tag_audio_events`, `remove_punctuation`; SRT formatting: `generate_srt`, `words_per_line`, `lines_per_subtitle`, `stretch_captions` |
...
|------|-------|---------|-------|
| `chat_send_message` | `POST /v1/chat` → polls status | 150s | Pass `session_id` back on follow-ups. `system_prompt` only applied on new sessions. |
...
| `restore_session` | `POST /v1/sessions/:sessionId/restore` |
