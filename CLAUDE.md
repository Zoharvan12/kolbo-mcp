...
| `generate_elements` | `POST /v1/generate/elements` | 600s | `reference_images`, `reference_videos`, `reference_audio_urls`, legacy `audio_url`, `files`, `visual_dna_ids`, `motion`, `preset_id`, `resolution`, `keyframes` (v1.56+), `multi_shots` / `multi_shot_count` / `session_name` (v1.65+) |
